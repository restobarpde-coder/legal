...
import { ArrowLeft, Edit, Users, FileText, CheckSquare, Clock, Mail, Phone, Building } from "lucide-react"
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" asChild>
                    <Link href={`/dashboard/cases/${case_.id}/tasks`}>
                      <CheckSquare className="h-4 w-4 mr-2" />
                      Tareas ({tasks.length})
                    </Link>
                  </Button>
                  <Button size="sm" variant="outline" asChild>
                    <Link href={`/dashboard/cases/${case_.id}/documents`}>
                ...
                {tasks.slice(0, 3).map((task) => (
                  <div key={task.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Tarea • {task.status}</p>
                    </div>
                ...
                {documents.slice(0, 2).map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Documento • {doc.document_type}</p>
                    </div>
